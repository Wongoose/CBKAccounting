...
// 9th May 2023 - UPDATE TRANSACTION CLOUD NOTES
exports.updateTransactionNotes = functions.https.onRequest(
	async (request, response) => {
		console.log("\nSTART OF updateTransactionNotes\n");
		...
		const ip_transid = request.query["ip_transid"] as string;
		const notes = request.query["notes"] as string;

		const { success, value } = await updateTransactionNotes(db, ip_transid, notes);
...
// Final test with cbkreconciliation web with dummy Xero invoices - DONE
