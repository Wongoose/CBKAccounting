...
export const updateTransactionNotes = async (
	...
	try {
		const snapshot = await firestore.collection("transactionLogs").where("ip_transid", "isEqualTo", ip_transid).get();
		
		snapshot.docs.forEach((doc) => {
			firestore.collection("transactionLogs").doc(doc.id).udpate({"notes": notes});
		});

		const result: ReturnValue = {
			success: true,
			value: `Updated notes for ${ip_transid}`,
			statusCode: 200,
		};
		return (result);
	} catch (err) {
